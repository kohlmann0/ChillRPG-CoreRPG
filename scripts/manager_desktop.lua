...
-- Make sure to set initialization flag before calling rebuild
...
function getSidebarWindow()
	if not _wShortcuts then
		...
		_wShortcuts.onLayoutSizeChanged = DesktopManager.onSidebarSizeChanged;
	end
	return _wShortcuts;
end
...
function configureSidebarTheming()
	...
	wShortcuts.button_visibility_icon.setColor(ColorManager.getSidebarRecordIconColor());
...
function rebuildSidebar()
	if not DesktopManager.isInitialized() then
		return;
	end
	...
	local tTool = DesktopManager.getSidebarToolButtons();
	...
	local tLibrary = DesktopManager.getSidebarLibraryButtons();
	local sLibraryCategory = DesktopManager.getSidebarLibraryCategory();
	for _,vButton in ipairs(tLibrary) do
		table.insert(tDockButtons, { sCategory = sLibraryCategory, sIcon = vButton.sIcon, sLabelRes = vButton.tooltipres, sClass = vButton.class, sPath = vButton.path });
	end
	...
	for _,sCategory in ipairs(_tSidebarCategories) do
		if tButtonsByCategory[sCategory] then
...
--
--  One-Off Tool/Library Registration
--

function getSidebarLibraryButtons()
	return Desktop.aCoreDesktopDockV4["live"];
end

function getSidebarToolButtons()
	if Session.IsHost then
		return Desktop.aCoreDesktopStack["host"];
	end
	return Desktop.aCoreDesktopStack["client"];
end
function registerSidebarToolButton(tButton, vInsertAt)
	local tTools = DesktopManager.getSidebarToolButtons();
	
	local nIndex = nil;
	if vInsertAt then
		local sIndexType = type(vInsertAt);
		if sIndexType == "number" then
			nIndex = vInsertAt;
		elseif sIndexType == "string" then
			for kTool,tTool in ipairs(tTools) do
				if tTool.class == vInsertAt then
					nIndex = kTool;
					break;
				end
			end
		end
	elseif tButton.sInsertBefore then
		for kTool,tTool in ipairs(tTools) do
			if tTool.class == tButton.sInsertBefore then
				nIndex = kTool;
				break;
			end
		end
	...
	if nIndex then
		table.insert(tTools, nIndex, tButton);
	else
		table.insert(tTools, tButton);
	end

	DesktopManager.rebuildSidebar();
end
function removeSidebarToolButton(sClass)
	local tTools = DesktopManager.getSidebarToolButtons();
	for kTool,tTool in ipairs(tTools) do
		if tTool.class == sClass then
			table.remove(tTools, kTool);
			DesktopManager.rebuildSidebar();
			break;
		end
	end
