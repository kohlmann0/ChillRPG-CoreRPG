...
function getRecordTypeFromWindow(w)
	return LibraryData.getRecordTypeFromClassAndPath(w.getClass(), w.getDatabasePath());
end
function getRecordTypeFromClassAndPath(sClass, sRecord)
	local tRecordTypes = LibraryData.getAllRecordTypesFromDisplayClass(sClass);
	if #tRecordTypes == 1 then
		return tRecordTypes[1];
	end
	return LibraryData.getRecordTypeFromRecordPath(sRecord);
end
...
function setHidden(sRecordType, bHidden)
	if aRecords[sRecordType] then
		if aRecords[sRecordType].bHidden ~= bHidden then
			aRecords[sRecordType].bHidden = bHidden;
...
function removeIndexButton(sRecordType, sButtonTemplate)
	...
	if Session.IsHost then
		if aRecords[sRecordType].aGMListButtons then
		...
		if aRecords[sRecordType].aPlayerListButtons then
...
function getAllRecordTypesFromDisplayClass(sClass)
	...
	for kRecordType,vRecordType in pairs(aRecords) do
		if LibraryData.isRecordDisplayClass(kRecordType, sClass) then
			table.insert(tResults, kRecordType);
		end
	end
	return tResults;
end
...
function getLockMode(sRecordType)
	if aRecords[sRecordType] then
		if aRecords[sRecordType].bNoLock then
			return false;
		end
	end
	return true;
...
function getShareMode(sRecordType)
	if aRecords[sRecordType] then
		if aRecords[sRecordType].bNoShare then
			return false;
		end
	end
	return true;
...
function allowCategories(sRecordType)
	return LibraryData.getCategoryMode(sRecordType);
end
...
function getCustomFilterValue(sKey, vRecord, rFilter)
	if aCustomFilterHandlers[sKey] then
		return aCustomFilterHandlers[sKey](vRecord, rFilter);
	end
	if rFilter then
