...
function setSpeakerIdentity(sIdentity)
	...
	if (sIdentity or "") ~= "" then
		DB.addHandler(DB.getPath("charsheet", sIdentity, CHAR_LANGUAGE_LIST), "onChildDeleted", LanguageManager.refreshChatLanguages);
		DB.addHandler(DB.getPath("charsheet", sIdentity, CHAR_LANGUAGE_LIST, "*", CHAR_LANGUAGE_LIST_NAME), "onUpdate", LanguageManager.refreshChatLanguages);
	end
