...
function onTabletopInit()
	ActionsManager.registerResultHandler("effect", ActionEffect.onEffect);
	Interface.addKeyedEventHandler("onHotkeyDrop", "", ActionEffect.onHotkeyDrop);
end
	...
	if rEffect.sSource == "" then
		...
		if sSourceCT == "" then
			...
			if Session.IsHost then
				...
				if sIdentity then
					nodeTempCT = CombatManager.getCTFromNode("charsheet." .. sIdentity);
				end
			end
			if nodeTempCT then
				sSourceCT = DB.getPath(nodeTempCT);
			end
		end
		if sSourceCT ~= "" then
