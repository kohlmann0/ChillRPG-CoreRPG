...
OOB_MSGTYPE_REMOVEEFF = "removeeff";
...
local aEffectVarMap = {
	...
	["nDuration"] = { sDBType = "number", sDBField = "duration", vDBDefault = 0, sDisplay = "[D: %d]" },
...
function onInit()
	CombatManager.setCustomInitChange(EffectManager.processEffects);
	
	OOBManager.registerOOBMsgHandler(EffectManager.OOB_MSGTYPE_APPLYEFF, EffectManager.handleApplyEffect);
	OOBManager.registerOOBMsgHandler(EffectManager.OOB_MSGTYPE_EXPIREEFF, EffectManager.handleExpireEffect);
	OOBManager.registerOOBMsgHandler(EffectManager.OOB_MSGTYPE_REMOVEEFF, EffectManager.handleRemoveEffect);
end
...
function notifyApply(rEffect, vTargets)
	...
	local msgOOB = {};
	msgOOB.type = EffectManager.OOB_MSGTYPE_APPLYEFF;
...
function notifyExpire(varEffect, nMatch, bImmediate)
	...
	local msgOOB = {};
	msgOOB.type = EffectManager.OOB_MSGTYPE_EXPIREEFF;
...
function notifyRemove(vTargets, sPattern)
	-- Build OOB message to pass effect to host
	local msgOOB = ChatManager.createBaseMessage();
	msgOOB.type = EffectManager.OOB_MSGTYPE_REMOVEEFF;
	msgOOB.sPattern = sPattern;
	...
	for k, v in pairs(aEffectVarMap) do
		if v.sDBField then
			...
			elseif ((v.sDBType == "string") and (rEffect[k] == "")) or ((v.sDBType == "number") and (rEffect[k] == 0)) then
				...
				DB.setValue(nodeEffect, v.sDBField, v.sDBType, rEffect[k]);
			end
		end
	end
end
	...
	for k, v in pairs(aEffectVarMap) do
		if v.sDBField then
			if v.sDBType == "number" then
				rEffect[k] = DB.getValue(nodeEffect, v.sDBField, v.vDBDefault or 0);
			else
				rEffect[k] = DB.getValue(nodeEffect, v.sDBField, v.vDBDefault or "");
			end
		end
	end
	return rEffect;
...
function message(sMsg, nodeCTEntry, bGMOnly, sUser)
	...
	elseif bGMOnly then
		...
		if Session.IsHost then
			...
			Comm.deliverChatMessage(msg, Session.UserName);
		end
	else
...
function removeEffect(nodeCTEntry, sEffPatternToRemove)
	...
	if _nDelayedUpdates > 0 then
		table.insert(_tRemoveAfterDelay, { sCTPath = DB.getPath(nodeCTEntry), sPattern = sEffPatternToRemove });
		return;
	end
...
function addEffect(sUser, sIdentity, nodeCT, rNewEffect, bShowMsg)
	...
	if _nDelayedUpdates > 0 then
		table.insert(_tAddAfterDelay, { sUser = sUser, sIdentity = sIdentity, sCTPath = DB.getPath(nodeCT), tEffect = rNewEffect, bShowMsg = bShowMsg });
		return;
	end

	local nodeEffectsList = DB.createChild(nodeCT, "effects");
	...
	if sDuplicateMsg then
		if bShowMsg then
			if bSecret then
				EffectManager.message(sDuplicateMsg, nodeCT, true);
			else
	...
	-- Write effect record
	local nodeTargetEffect = DB.createChild(nodeEffectsList);
	for k,v in pairs(aEffectVarMap) do
	...
	if bShowMsg then
		if bSecret then
			Comm.addChatMessage(msg);
		elseif CombatManager.isCTHidden(nodeCT) then
			if sUser == "" then
				msg.secret = true;
				Comm.addChatMessage(msg);
			elseif sUser ~= "" then
...
function expireEffect(nodeActor, nodeEffect, nExpireComp)
	...
	if _nDelayedUpdates > 0 then
		table.insert(_tExpireAfterDelay, { sActorPath = DB.getPath(nodeActor), sEffectPath = DB.getPath(nodeEffect), nExpireComp = nExpireComp });
		return true;
	end
...
function setEffectSource(nodeEffect, nodeCT)
	...
	local sSourcePath = DB.getPath(nodeCT);
	if DB.getPath(nodeEffectActor) == sSourcePath then
...
function isEffectTarget(nodeEffect, rTarget)
	...
	if sTargetCT ~= "" then
		for _,v in ipairs(DB.getChildList(nodeEffect, "targets")) do
...
function getEffectTargets(nodeEffect, bUseName)
	...
	for _,nodeTarget in ipairs(DB.getChildList(nodeEffect, "targets")) do
...
function addEffectTarget(vEffect, sTargetNode)
	...
	for _,nodeTarget in ipairs(DB.getChildList(nodeTargetList)) do
		if (DB.getValue(nodeTarget, "noderef", "") == sTargetNode) then
			return;
		end
	end

	local nodeNewTarget = DB.createChild(nodeTargetList);
...
function setEffectFactionTargets(nodeEffect, sFaction, bNegated)
	...
	local sTrackerKey = CombatManager.getTrackerKeyFromCT(nodeEffect);
	...
	for _,nodeCT in pairs(CombatManager.getCombatantNodes(sTrackerKey)) do
		if bNegated then
			if CombatManager.getFactionFromCT(nodeCT) ~= sFaction then
				EffectManager.addEffectTarget(nodeEffect, DB.getPath(nodeCT));
			end
		else
			if CombatManager.getFactionFromCT(nodeCT) == sFaction then
				EffectManager.addEffectTarget(nodeEffect, DB.getPath(nodeCT));
...
function clearEffectTargets(nodeEffect)
	DB.deleteChildren(nodeEffect, "targets");
end
...
function encodeEffectAsText(rEffect)
	...
	if rEffect then
		table.insert(aMessage, string.format("[%s] %s", Interface.getString("effect_tag"), rEffect.sName));
...
function buildEffectOutput(v)
	if (type(v) == "number") then
		if v ~= 0 then
			return string.format("[%s %+d]", Interface.getString("effects_tag"), v);
		end
	else
		if v ~= "" then
			return string.format("[%s %s]", Interface.getString("effects_tag"), v);
		end
	end
	return string.format("[%s]", Interface.getString("effects_tag"));
end
...
function hasCondition(rActor, sEffect)
	return EffectManager.hasEffect(rActor, sEffect);
end

function addCondition(rActor, sEffect)
	if (sEffect or "") == "" then
		return;
	end
	if EffectManager.hasEffect(rActor, sEffect) then 
		return; 
	end
	
	local tEffect = { sName = sEffect, nDuration = 0 };
	if not ActorManager.isFaction(rActor, "friend") then
		tEffect.nGMOnly = 1;
	end
	EffectManager.notifyApply(tEffect, ActorManager.getCTNodeName(rActor));
end

function removeCondition(rActor, sEffect)
	if EffectManager.hasEffect(rActor, sEffect) then 
		EffectManager.notifyRemove(ActorManager.getCTNodeName(rActor), sEffect);
	end
end
