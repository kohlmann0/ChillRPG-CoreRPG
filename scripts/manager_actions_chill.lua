...
-- NOTE: This is a clone of the original coreRPG dice roller.
-- This is a custom implementation of it for the ChillRPG 3E
--	The new mechanics is a Roll-Under system, on a single d100, with some special bonus's 
...
function createActionMessage(rSource, rRoll)
	...
	local total = tonumber(rRoll.nTotal);
	
	local chillDescription = rRoll.sDesc .. "\r\n";
	if total % 11 == 0 and total > target then
		chillDescription = chillDescription .. "CRITICAL FAILURE!";
	elseif total % 11 == 0 and total <= target then
		chillDescription = chillDescription .. "CRITICAL SUCCESS!";		
	elseif total <= target and total > (target / 2) then
		chillDescription = chillDescription .. "High Success!";		
	elseif total <= (target / 2) then
		chillDescription = chillDescription .. "Success.";		
	elseif total > target then
		chillDescription = chillDescription .. "Failure.";
	end

	rMessage.type = rRoll.sType;
	rMessage.text = chillDescription;
