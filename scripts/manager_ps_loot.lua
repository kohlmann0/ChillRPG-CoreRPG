...
function populate()
	if DB.getChildCount("partysheet.treasureparcelcoinlist") == 0 then
		for _,sCurrency in ipairs(CurrencyManager.getCurrencies()) do
			local node = DB.createChild("partysheet.treasureparcelcoinlist");
			DB.setValue(node, "description", "string", sCurrency);
		end
	end
		...
		if sItem ~= "" and nCount > 0 then
			...
			if sAssign ~= "" then
				...
				if #aSplit > nCount then
					local sMsg = string.format("[%s] %s (%s)", Interface.getString("tag_warning"), Interface.getString("ps_message_itemfailtoomanyassign"), sAssign);
				...
				for _,vAssign in ipairs(aSplit) do
					...
					for _,rActor in ipairs(tParty) do
						if vAssign:lower() == ActorManager.getDisplayName(rActor):lower() then
							rTarget = rActor;
						end
					end
					if rTarget then
				...
				if #aAssigned > 0 then
					...
					if #aFailedAssign > 0 then
						...
						local sMsg = string.format("[%s] %s (%s) (%s)", Interface.getString("tag_warning"), Interface.getString("ps_message_itemfailcreate"), sItem, sFailedAssign);
						ChatManager.SystemMessage(sMsg);
					end
				end
			end
...
function buildPartyCoins()
	...
	for _,v in ipairs(tParty) do
		local sRecordType = ActorManager.getRecordType(v.node);
		local tCurrencyPaths = ItemManager.getCurrencyPaths(sRecordType) or {};
		for _,sPath in ipairs(tCurrencyPaths) do
			for _,nodeCoin in ipairs(DB.getChildList(v.node, sPath)) do
...
function addPartyMemberItem(rActor, nodeItem, nAssign)
	local sList = ItemManager.getTargetInventoryListPath(rActor.node);
	if not sList then
		return nil;
	end
	return ItemManager.addItemToList(DB.getPath(rActor.node, sList), "item", nodeItem, false, nAssign);
end
function addPartyMemberCurrency(rActor, sCoin, nCoin)
	if ActorManager.isPC(rActor) then
		CurrencyManager.addActorCurrency(rActor.node, sCoin, nCoin);
	end
end
